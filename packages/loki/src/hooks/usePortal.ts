 ...
 * @returns {HTMLElement}
 */
function createRootElement(id): Element {
...
function addRootElement(rootElem): void {
  const { nextElementSibling } = document.body.lastElementChild || ({ nextElementSibling: null } as Element);
  document.body.insertBefore(rootElem, nextElementSibling);
}

function checkIsRemovePropertyExisted(element): boolean {
...
function usePortal(id: string): Element {
  ...
  useEffect(() => {
    ...
    return function removeElement() {
      ...
      if (parentElem.childNodes.length === 0 && checkIsRemovePropertyExisted(parentElem)) {
        parentElem.remove();
      }
    };
  }, [id]);
