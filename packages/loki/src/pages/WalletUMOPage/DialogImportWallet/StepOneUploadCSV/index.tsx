...
import DownloadTemplateButton, { LINK_TEMPLATE_CSV_CRYPTO } from 'components/Wallets/DownloadTemplateButton';

import { isVerifying } from 'redux/features/walletBanks/slice';
import { validateCryptoWalletsThunk } from 'redux/features/walletUMO/thunks';
...
interface Props {
  ...
  onBack: () => void;
  onFirst: () => void;
  onFileListChange?: (uploadedFile: File, data: CryptoWallet.AsObject[]) => void;
...
const validateRow = (wallet: CryptoWallet.AsObject) => {
...
const StepOneUploadCSV = (props: Props) => {
  ...
  const [list, setList] = useState<CryptoWallet.AsObject[]>([...(defaultValue ? defaultValue.list : [])]);
    ...
    if (isNotCorrectFile || isNotMatchColumns || isInvalidData) {
      ...
      setError({
        ...
        message: isNotCorrectFile
          ...
          : isNotMatchColumns
          ? t(`File columns don't match. Please check again`)
          : isInvalidData
    ...
    <Grid container direction="column" spacing={3}>
      ...
      <Grid item>
        <Box pb={2} display="flex" alignItems="center" flexDirection="column">
          ...
          <Box mt={2}>
            <DownloadTemplateButton hrefLink={LINK_TEMPLATE_CSV_CRYPTO} />
          </Box>
        </Box>
      </Grid>
    </Grid>
