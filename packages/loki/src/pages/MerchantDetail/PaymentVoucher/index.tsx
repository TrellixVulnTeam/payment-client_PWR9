...
import { selectVoucherStatus, selectTotalRecord, selectVouchers } from 'redux/features/vouchers/slice';
...
import { APP_NAME, formatCurrency } from 'utils/common';
import { formatWithSchema } from 'utils/date';
import Filter from '../ReceiptVoucher/Filter';
import CreateVoucher from '../ReceiptVoucher/CreateVoucher';
import useFetchVoucher from '../ReceiptVoucher/useFetchVoucher';
import { DEFAULT_PAGE_SIZE } from '../ReceiptVoucher/const';
import VoucherStatus from '../VoucherStatus';
import { PaymentVoucherStatuses, PaymentVoucherTypes } from './const';
...
const PaymentVoucher = (props: Props) => {
  ...
  const [reloadAPI] = useFetchVoucher(PaymentVoucherTypes);
  ...
  useEffect(() => {
    if (!!merchant) {
      const breadcrumbs: IBreadcrumb[] = [
        ...
        {
          to: ``,
          label: `${t('Payment Voucher Detail')}`,
          active: true,
        },
      ];
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: (row: Voucher.AsObject) => PaymentVoucherTypes.find((item) => item.value === row.type)?.name || '-',
    },
    {
      Header: t('Created by'),
  ...
  return (
    <div className={styles['root']}>
      <Helmet>
        <title>
          {t('Payment Voucher Detail')} - {APP_NAME}
        </title>
      </Helmet>
      <LayoutContainer
        header={
          <MerchantHeader
            merchant={{ ...merchant, name: `${merchant?.name || ''} - ${t('Payment Voucher Detail')}` }}
          />
        }
        maxWidth={false}
      >
        <Grid container className={styles.root} spacing={4}>
          <Grid item xs={12}>
            <Grid container alignItems="center" justifyContent="space-between">
              <Grid item xs={true}>
                <Filter VoucherOptions={PaymentVoucherTypes} voucherStatusOptions={PaymentVoucherStatuses} />
              </Grid>
              <AllowedTo perform={PerformPermission.merchantPayment.createVoucher} watch={[reloadData]}>
                <Grid item xs="auto">
                  <CreateVoucher isPaymentVoucher VoucherOptions={PaymentVoucherTypes} callback={reloadData} />
                </Grid>
              </AllowedTo>
            </Grid>
          </Grid>
          <Grid item xs={12}>
            <AlopayTable
              ...
              tableRowProps={{
                onClick: (row) => {
                  history.push(`/merchant/${merchant.id}/payment-detail/${row.id}`);
                },
              }}
            />
          </Grid>
